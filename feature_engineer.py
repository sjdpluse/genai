"""
feature_engineer.py — 80+ Advanced Features (CoinGecko Edition)
"""
import pandas as pd
import numpy as np
import ta
...
def compute_indicators(df: pd.DataFrame) -> pd.DataFrame:
    ...
    n_rows = len(df)
    logger.info(f"Computing indicators on {n_rows} rows")
...
def create_labels(df_1h: pd.DataFrame,
    ...
    logger.info(f"Labels: LONG={n_long}, SHORT={n_short}, WAIT={n_wait}, NaN={n_nan}")
