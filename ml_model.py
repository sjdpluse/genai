...
def _purge_train_indices(train_idx: np.ndarray, embargo: int = EMBARGO) -> np.ndarray:
    """حذف آخرین embargo ردیف از پنجره آموزشی (جلوگیری از leakage)"""
...
def _build_pipeline() -> Pipeline:
    """
    فکتوری Pipeline — Ensemble از ۳ مدل sklearn (بدون نیاز به lib سیستمی)
    """
...
def train_model(X: pd.DataFrame, y: pd.Series) -> tuple:
    ...
    if len(X_clean) < 500:
        raise ValueError(
            ...
            "limit را در run_training افزایش دهید."
        )
    ...
    for fold, (train_idx, test_idx) in enumerate(tscv.split(X_clean)):
        ...
        fold_f1s.append(f1)
        logger.info(f"Fold {fold+1}: accuracy={score:.3f}, f1={f1:.3f} (embargo={EMBARGO})")
    ...
    # Feature Importance (از RandomForest داخل ensemble)
    try:
